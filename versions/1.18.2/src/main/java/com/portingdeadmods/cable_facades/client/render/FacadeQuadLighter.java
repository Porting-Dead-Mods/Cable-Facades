...
public final class FacadeQuadLighter {
    ...
    public static void renderQuad(BlockAndTintGetter level, BlockState state, BlockPos pos,
        ...
        if (useAo) {
            ...
            buffer.putBulkData(pose, quad, aoFace.brightness, red, green, blue, aoFace.lightmap, overlay, true);
            return;
        }
        ...
        buffer.putBulkData(pose, quad, new float[]{shade, shade, shade, shade}, red, green, blue,
    ...
    private static final class AmbientOcclusionFace {
        ...
        private void calculate(BlockAndTintGetter level, BlockState state, BlockPos pos, Direction direction,
            ...
            float cornerBrightness0; int cornerLight0;
            if (!open2 && !open0) { cornerBrightness0 = bright0; cornerLight0 = light0; }
            else { cursor.setWithOffset(basePos, adjacency.corners[0]).move(adjacency.corners[2]); BlockState corner = level.getBlockState(cursor); cornerBrightness0 = corner.getShadeBrightness(level, cursor); cornerLight0 = LevelRenderer.getLightColor(level, corner, cursor); }

            float cornerBrightness1; int cornerLight1;
            if (!open3 && !open0) { cornerBrightness1 = bright0; cornerLight1 = light0; }
            else { cursor.setWithOffset(basePos, adjacency.corners[0]).move(adjacency.corners[3]); BlockState corner = level.getBlockState(cursor); cornerBrightness1 = corner.getShadeBrightness(level, cursor); cornerLight1 = LevelRenderer.getLightColor(level, corner, cursor); }

            float cornerBrightness2; int cornerLight2;
            if (!open2 && !open1) { cornerBrightness2 = bright0; cornerLight2 = light0; }
            else { cursor.setWithOffset(basePos, adjacency.corners[1]).move(adjacency.corners[2]); BlockState corner = level.getBlockState(cursor); cornerBrightness2 = corner.getShadeBrightness(level, cursor); cornerLight2 = LevelRenderer.getLightColor(level, corner, cursor); }

            float cornerBrightness3; int cornerLight3;
            if (!open3 && !open1) { cornerBrightness3 = bright0; cornerLight3 = light0; }
            else { cursor.setWithOffset(basePos, adjacency.corners[1]).move(adjacency.corners[3]); BlockState corner = level.getBlockState(cursor); cornerBrightness3 = corner.getShadeBrightness(level, cursor); cornerLight3 = LevelRenderer.getLightColor(level, corner, cursor); }
    ...
    private enum AmbientVertexRemap {
        ...
        AmbientVertexRemap(int vert0, int vert1, int vert2, int vert3) {
            this.vert0 = vert0; this.vert1 = vert1; this.vert2 = vert2; this.vert3 = vert3;
        }
    ...
    private enum SizeInfo {
        DOWN(Direction.DOWN, false), UP(Direction.UP, false),
        NORTH(Direction.NORTH, false), SOUTH(Direction.SOUTH, false),
        WEST(Direction.WEST, false), EAST(Direction.EAST, false),
        FLIP_DOWN(Direction.DOWN, true), FLIP_UP(Direction.UP, true),
        FLIP_NORTH(Direction.NORTH, true), FLIP_SOUTH(Direction.SOUTH, true),
        FLIP_WEST(Direction.WEST, true), FLIP_EAST(Direction.EAST, true);
    ...
    private enum AdjacencyInfo {
        DOWN(
                new Direction[]{Direction.WEST, Direction.EAST, Direction.NORTH, Direction.SOUTH}, true,
        ...
        UP(
                new Direction[]{Direction.EAST, Direction.WEST, Direction.NORTH, Direction.SOUTH}, true,
        ...
        NORTH(
                new Direction[]{Direction.UP, Direction.DOWN, Direction.EAST, Direction.WEST}, true,
        ...
        SOUTH(
                new Direction[]{Direction.WEST, Direction.EAST, Direction.DOWN, Direction.UP}, true,
        ...
        WEST(
                new Direction[]{Direction.UP, Direction.DOWN, Direction.NORTH, Direction.SOUTH}, true,
        ...
        EAST(
                new Direction[]{Direction.DOWN, Direction.UP, Direction.NORTH, Direction.SOUTH}, true,
        ...
        AdjacencyInfo(Direction[] corners, boolean doNonCubicWeight, SizeInfo[] vert0Weights, SizeInfo[] vert1Weights,
            ...
            this.vert0Weights = vert0Weights; this.vert1Weights = vert1Weights;
            this.vert2Weights = vert2Weights; this.vert3Weights = vert3Weights;
        }
