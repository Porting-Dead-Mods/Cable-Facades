...
public final class ChunkFacadeBaker {
    ...
    private static void bake(BlockPos sectionOrigin, BlockAndTintGetter region,
        ...
        BlockAndTintGetter facadeAware = new FacadeAwareBlockGetter(region);
        ...
        try {
            ...
            ClientFacadeManager.entryStream()
                    ...
                    .forEach(entry -> {
                        ...
                        if (data == null) return;
                        if (data.isFullBlock()) {
                            renderFullBlockFacade(facadeAware, bufferProvider, random, pos, data.getFullBlock());
                        } else if (data.isDirectional()) {
                            data.directional().forEach((dir, state) ->
                                    renderDirectionalFacade(facadeAware, bufferProvider, random, pos, dir, state));
                        }
                    });
        } finally {
    ...
    private static void renderFullBlockFacade(BlockAndTintGetter region,
        ...
        Block facadedBlock = unwrap(region).getBlockState(pos).getBlock();
        ...
        for (RenderType renderType : getRenderTypes(facadeState, facadeModel)) {
    ...
    private static List<RenderType> getRenderTypes(BlockState state, BakedModel model) {
        List<RenderType> types = new java.util.ArrayList<>();
        ...
        if (types.isEmpty()) {
            types.add(ItemBlockRenderTypes.getChunkRenderType(state));
        }
        return types;
    }

    private static BlockAndTintGetter unwrap(BlockAndTintGetter region) {
        if (region instanceof FacadeAwareBlockGetter facade) {
