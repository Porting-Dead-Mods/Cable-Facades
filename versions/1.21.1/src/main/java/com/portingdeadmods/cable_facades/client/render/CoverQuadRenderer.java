...
public final class CoverQuadRenderer {
    ...
    public static List<BakedQuad> sliceQuads(BlockState state, BlockPos pos, BakedModel model,
        ...
        return sliceQuads(state, pos, model, side, modelData, null);
    }

    public static List<BakedQuad> sliceQuads(BlockState state, BlockPos pos, BakedModel model,
                                             Direction side, ModelData modelData, RenderType renderType) {
        long seed = state.getSeed(pos);
        AABB bounds = COVER_BOXES[side.ordinal()];
        List<BakedQuad> sourceQuads = new ArrayList<>();

        sourceQuads.addAll(model.getQuads(state, null, RandomSource.create(seed), modelData, renderType));
        for (Direction face : Direction.values()) {
            sourceQuads.addAll(model.getQuads(state, face, RandomSource.create(seed), modelData, renderType));
        }
